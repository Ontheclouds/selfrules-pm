...
<div id="options">
			...
			<?php if($invoice->status != "Paid" && isset($invoice->company->name)){ ?><a href="<?=base_url()?>invoices/sendinvoice/<?=$invoice->id;?>" class="btn"><i class="icon-envelope"></i> <?=$this->lang->line('application_send_invoice_to_client');?></a><?php } ?>
            	...
            	<a href="<?=base_url()?>invoice_pagata.php?invoice_id=<?=$invoice->id;?>&status=<?php echo $invoice->status == "Paid" ? 'paid' : null ?>" class="btn" data-toggle="modal"><i class="icon-edit"></i> Segna come  <?php if($invoice->status == "Paid") { ?>non <?php } ?>pagata</a>
		</div>
		<hr>
		<div class="row">
